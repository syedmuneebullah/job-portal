{{-- resources/views/jobseeker/pages/job-details.blade.php --}}
...
<div class="bg-slate-50/50 min-h-screen py-8">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <nav class="flex items-center gap-2 text-sm mb-6" aria-label="Breadcrumb">
            ...
            <a href="{{ route('candidate.jobs.listings') }}" class="text-gray-500 hover:text-[#1a237e] transition-colors">Jobs</a>
            <i class="fas fa-chevron-right text-[10px] text-gray-400"></i>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div class="lg:col-span-2 space-y-6">
                ...
                <div class="bg-white rounded-2xl shadow-sm border border-gray-100/80 p-6 md:p-8">
                    <div class="flex flex-col md:flex-row md:items-start md:justify-between gap-4">
                                ...
                                <div class="flex flex-wrap items-center gap-2 mt-1.5">
                                    ...
                                    @if($job->closing_at)
                                        ...
                                        <span class="text-sm text-amber-600 flex items-center gap-1">
                                            <i class="fas fa-hourglass-end text-[#ff7543] text-xs"></i>
                                            Closing {{ $job->closing_at->diffForHumans() }}
                                        </span>
                                    @endif
                                </div>
                            </div>
                        ...
                        <div class="flex flex-wrap items-center gap-2">
                            ...
                            @if($job->is_ai_generated)
                                <span class="px-3 py-1.5 bg-purple-50 text-purple-700 rounded-full text-xs font-semibold">
                    ...
                    <div class="flex flex-wrap gap-2 mt-4 pt-4 border-t border-gray-100">
                        ...
                        @if($job->work_type)
                            <span class="text-xs font-medium 
                                ...
                                <i class="fas fa-laptop-house mr-1 text-[10px]"></i>
                        ...
                        @if($job->salary_min && $job->salary_max)
                            <span class="text-xs font-medium bg-purple-50 text-purple-700 px-3 py-1.5 rounded-full">
                                <i class="fas fa-money-bill-wave mr-1 text-[10px]"></i>
                        ...
                        @if($job->max_applications)
                            <span class="text-xs font-medium bg-cyan-50 text-cyan-700 px-3 py-1.5 rounded-full">
                                <i class="fas fa-users mr-1 text-[10px]"></i>
                                Max {{ $job->max_applications }} applicants
                            </span>
                        @endif
                    </div>
                    ...
                    <div class="flex flex-wrap gap-3 mt-5 pt-5 border-t border-gray-100">
                        ...
                        @if($hasApplied)
                            <div class="flex-1 md:flex-none px-8 py-3 bg-emerald-50 text-emerald-700 font-semibold rounded-xl border-2 border-emerald-200 flex items-center justify-center gap-2 cursor-default">
                                <i class="fas fa-check-circle"></i>
                                Already Applied
                            </div>
                        @elseif(auth()->check() && auth()->user()->applicantProfile && auth()->user()->applicantProfile->resume_path)
                            ...
                            <a href="{{ route('candidate.job.apply.form', $job->id) }}" 
                        ...
                        <button onclick="toggleSaveJob({{ $job->id }}, this)" 
                            ...
                            <i class="{{ $isSaved ? 'fas' : 'far' }} fa-bookmark {{ $isSaved ? 'text-[#ff7543]' : 'text-gray-400' }}"></i>
                            <span>{{ $isSaved ? 'Saved' : 'Save' }}</span>
                        </button>
                        
                        <button onclick="shareJob({{ $job->id }}, '{{ $job->title }}')" 
                                class="px-5 py-3 border-2 border-gray-200 hover:border-[#1A237E] text-gray-600 hover:text-[#1A237E] font-semibold rounded-xl transition-all duration-300 flex items-center gap-2 hover:bg-gray-50">
                    ...
                    <div class="mb-4">
                        ...
                        <div class="flex flex-wrap gap-2">
                            @foreach($requiredSkills as $skill)
                                <span class="text-xs font-medium bg-blue-50 text-blue-700 px-3 py-1.5 rounded-full border border-blue-100">
                                    {{ $skill }}
                                </span>
                            @endforeach
                        </div>
                    ...
                    <div>
                        ...
                        <div class="flex flex-wrap gap-2">
                            @foreach($preferredSkills as $skill)
                                <span class="text-xs font-medium bg-purple-50 text-purple-700 px-3 py-1.5 rounded-full border border-purple-100">
                                    {{ $skill }}
                                </span>
                            @endforeach
                        </div>
                        ...
                        <div>
                            ...
                            @if($job->employer->industry)
                                <p class="text-sm text-gray-600 mt-1">
                                    <i class="fas fa-industry text-[#ff7543] text-xs mr-1"></i>
                                    {{ $job->employer->industry }}
                                </p>
                            @endif
                            @if($job->employer->description)
                                <p class="text-sm text-gray-600 mt-2 max-w-2xl">{{ $job->employer->description }}</p>
                            @endif
                    ...
                    <div class="space-y-3">
                        ...
                        <div class="group flex flex-col sm:flex-row sm:items-center justify-between p-4 rounded-xl border border-gray-100 hover:border-[#1A237E]/20 hover:shadow-md transition-all duration-300">
                            <div class="flex items-start gap-3">
                                <div class="w-10 h-10 rounded-xl bg-gray-100/70 flex items-center justify-center text-[#1A237E] text-sm font-bold shrink-0 overflow-hidden">
                                    @if($similarJob->employer && $similarJob->employer->company_logo)
                                        <img src="{{ Storage::url($similarJob->employer->company_logo) }}" 
                                             alt="{{ $similarJob->employer->company_name }}" 
                                             class="w-full h-full object-cover">
                                    @else
                                ...
                                <div>
                                    <h4 class="text-sm font-bold text-gray-900 group-hover:text-[#1A237E] transition-colors">
                                        <a href="{{ route('candidate.job.details', $similarJob->id) }}">{{ $similarJob->title }}</a>
                                    </h4>
                                    ...
                                    <div class="flex flex-wrap gap-1 mt-1">
                                        @if($similarJob->employment_type)
                                            <span class="text-[10px] font-medium bg-emerald-50 text-emerald-700 px-2 py-0.5 rounded-full">
                                                {{ ucfirst(str_replace('_', ' ', $similarJob->employment_type)) }}
                                            </span>
                                        @endif
                                        @if($similarJob->work_type)
                            ...
                            <div class="flex items-center gap-4 mt-2 sm:mt-0">
                                ...
                                @endif
                                <a href="{{ route('candidate.job.details', $similarJob->id) }}" 
            ...
            <div class="space-y-6">
                ...
                <div class="bg-gradient-to-br from-[#1A237E]/5 to-[#1A237E]/10 rounded-2xl border-2 border-[#1A237E]/20 p-6">
                    ...
                    @if($hasApplied)
                        <div class="w-full py-3 bg-emerald-50 text-emerald-700 font-semibold rounded-xl border-2 border-emerald-200 flex items-center justify-center gap-2 cursor-default">
                            <i class="fas fa-check-circle"></i>
                            Already Applied
                        </div>
                    @elseif(auth()->check() && auth()->user()->applicantProfile && auth()->user()->applicantProfile->resume_path)
                        <button onclick="quickApply({{ $job->id }})" 
                                class="w-full py-3 bg-[#1A237E] hover:bg-[#0D1445] text-white font-semibold rounded-xl transition-all duration-300 shadow-lg shadow-[#1A237E]/20 hover:shadow-xl hover:shadow-[#1A237E]/30 flex items-center justify-center gap-2">
                            <i class="fas fa-paper-plane"></i>
                            Quick Apply
                        </button>
                    @else
                    ...
                    <div class="mt-3 text-center">
                        <a href="{{ route('candidate.profile.edit') }}" class="text-xs text-[#1A237E] hover:text-[#0D1445] font-medium transition-colors">
                        ...
                        <div class="flex items-start gap-3">
                            ...
                            <div>
                                ...
                                @if($job->closing_at->diffInDays(now()) <= 7)
                                    <span class="text-xs text-amber-600 block">Closing soon!</span>
                                @endif
                            </div>
                        </div>
                        @endif
                        <div class="flex items-start gap-3">
                            <i class="fas fa-chart-line text-[#1A237E] text-sm mt-0.5 w-5"></i>
                            <div>
                ...
                <div class="bg-white rounded-2xl shadow-sm border border-gray-100/80 p-6">
                    ...
                    <div class="flex gap-2">
                        <button onclick="shareJob({{ $job->id }}, '{{ $job->title }}', 'linkedin')" 
                                class="flex-1 p-2.5 rounded-xl border border-gray-200 hover:border-[#0a66c2] hover:bg-[#0a66c2]/10 transition-all duration-300 text-center text-gray-600 hover:text-[#0a66c2]">
                            <i class="fab fa-linkedin-in"></i>
                        </button>
                        <button onclick="shareJob({{ $job->id }}, '{{ $job->title }}', 'twitter')" 
                                class="flex-1 p-2.5 rounded-xl border border-gray-200 hover:border-[#1DA1F2] hover:bg-[#1DA1F2]/10 transition-all duration-300 text-center text-gray-600 hover:text-[#1DA1F2]">
                            <i class="fab fa-twitter"></i>
                        </button>
                        <button onclick="shareJob({{ $job->id }}, '{{ $job->title }}', 'facebook')" 
                                class="flex-1 p-2.5 rounded-xl border border-gray-200 hover:border-[#1877F2] hover:bg-[#1877F2]/10 transition-all duration-300 text-center text-gray-600 hover:text-[#1877F2]">
                            <i class="fab fa-facebook-f"></i>
                        </button>
                        <button onclick="shareJob({{ $job->id }}, '{{ $job->title }}', 'copy')" 
                                class="flex-1 p-2.5 rounded-xl border border-gray-200 hover:border-[#1A237E] hover:bg-[#1A237E]/10 transition-all duration-300 text-center text-gray-600 hover:text-[#1A237E]">
                ...
                <!-- ===== REPORT JOB ===== -->
                <a href="#" class="flex items-center justify-center gap-2 text-sm text-gray-400 hover:text-[#1A237E] transition-colors p-3">
...
<script>
    ...
    function quickApply(jobId) {
        Swal.fire({
            title: 'Quick Apply?',
            text: 'You are about to apply for this position using your saved profile.',
            icon: 'question',
    ...
    function shareJob(jobId, jobTitle, platform = 'copy') {
