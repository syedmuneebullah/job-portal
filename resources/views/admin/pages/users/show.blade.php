...
<div class="space-y-6">
            ...
            <div class="flex items-end gap-6 -mt-16">
                <div class="relative">
                    <div class="w-28 h-28 rounded-2xl border-4 border-white bg-white shadow-xl overflow-hidden">
                        ...
                        @else
                            <div class="w-full h-full bg-gradient-to-br from-[#1a237e] to-[#FF7543] flex items-center justify-center">
                    ...
                    <div class="flex flex-wrap items-start justify-between gap-4">
                        ...
                        <div class="flex flex-wrap items-center gap-2">
                            ...
                            <span class="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-xs font-semibold
                                @if($user->user_type === 'admin') bg-purple-100 text-purple-700
                                @elseif($user->user_type === 'employer') bg-orange-100 text-orange-700
                                @elseif($user->user_type === 'recruiter') bg-blue-100 text-blue-700
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
            ...
            <div class="p-4 space-y-3">
                @forelse($user->applications as $application)
                    <div class="group flex items-center justify-between p-3 rounded-xl hover:bg-gray-50 transition-all duration-200 cursor-pointer">
                        <div class="flex items-center gap-3">
                            <div class="w-10 h-10 rounded-xl bg-gray-100 group-hover:bg-blue-50 transition-colors flex items-center justify-center">
                            ...
                            <div>
                                <p class="text-sm font-medium text-gray-900 group-hover:text-[#1a237e] transition-colors">
                ...
                @empty
                    <div class="text-center py-8">
                        ...
                        <p class="text-sm text-gray-500 mt-3">No applications submitted</p>
                    </div>
                @endforelse
            </div>
        </div>
    ...
    <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        @if(!$user->email_verified_at)
            ...
            <form id="verify-form-{{ $user->id }}" action="{{ route('admin.users.verify-email', $user->id) }}" method="POST" class="hidden">
                @csrf
            </form>
        @endif
