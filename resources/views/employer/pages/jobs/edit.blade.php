...
<div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
    ...
    <form action="{{ route('employer.jobs.update', $job->id) }}" method="POST" class="space-y-6" id="jobForm">
        @csrf
        @method('PUT')
            ...
            <div class="border border-gray-200 rounded-xl overflow-hidden">
                ...
                <div class="p-6">
                    <div id="questions-container" class="space-y-4">
                        @php
                            $questions = old('questions', $job->questions->toArray());
                        @endphp
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    let questionCounter = {{ count($job->questions) }};
    ...
    document.querySelectorAll('.question-item').forEach(function(item) {
        ...
        removeBtn.addEventListener('click', function() {
            // Check if this question has an ID (existing question)
            const idInput = item.querySelector('input[name*="[id]"]');
            if (idInput && idInput.value) {
                // For existing questions, add ID to deleted list
                const deletedInput = document.createElement('input');
                deletedInput.type = 'hidden';
                deletedInput.name = 'deleted_question_ids[]';
                deletedInput.value = idInput.value;
                document.getElementById('jobForm').appendChild(deletedInput);
            }
            item.remove();
            updateQuestionNumbers();
