...
class ResumeController extends Controller
{
    ...
    public function upload(Request $request)
    {
        ...
        // If service returned a model with failed status
        if ($resumeParse && $resumeParse->status === 'failed') {
            $error = $resumeParse->parsed_data['error'] ?? 'Unknown parsing error.';

            return redirect()->back()
                ->with('error', 'Failed to parse resume: ' . $error);
        }
